...
shuric_t *
shuric_load (shuric_vtable_t * vt, caddr_t uri, caddr_t ts, caddr_t uri_text_content, shuric_t * loaded_by,
    struct query_instance_s *qi, void *env, caddr_t * err_ret)
{
  caddr_t str = NULL;
  ...
  dbg_printf (("shuric_load started (\"%s\", \"%s\", \"%s\" @ %p)\n",
	  vt->shuric_type_title, uri, loaded_by ? loaded_by->shuric_uri : "", loaded_by));
  if (NULL != err_ret[0])
    return NULL;
  if (NULL != uri)
    {
	...
	{
	  err_ret[0] =
	      srv_make_new_error ("39000", "FA042",
	      "An abnormally long string is passed as URI of a shareable XSLT/XQuery resource.");
	  goto cleanup;
	}
    }
  if (NULL != uri)
      ...
      if (NULL != loaded_by)
	{
	  ...
	  if (old_inc != NULL)
	      ...
	      else
		{
	      ...
	      err_ret[0] =
		  srv_make_new_error ("39000", "FA048",
		  "Cyclic references: '%.300s' refers to his ancestor '%.300s' as to child resource", impuri, uri);
	    }
	}
	...
	{
	  err_ret[0] =
	      srv_make_new_error ("39000", "FA046",
	      "Server uses the content of URI '%.200s' as '%.30s' and can not load it as '%.30s'", uri,
	      old_cached_shuric->_->shuric_type_title, vt->shuric_type_title);
	  goto cleanup;
	}
    }
  ...
  dbg_printf (("  new_shuric = %p state=\"%s\" err=\"%s\"\n", res,
	  err_ret[0] ? ERR_STATE (err_ret[0]) : "00000", err_ret[0] ? ERR_MESSAGE (err_ret[0]) : "OK"));
  if (NULL != err_ret[0])
    {
      goto cleanup;
    }
  ...
  if (NULL != uri)
      ...
      if ((NULL != new_cached_shuric) && (new_cached_shuric->_ != vt))
	{
	  err_ret[0] =
	      srv_make_new_error ("39000", "FA046",
	      "Server uses the content of URI '%.200s' as '%.30s' and can not load it as '%.30s'", uri,
	      new_cached_shuric->_->shuric_type_title, vt->shuric_type_title);
	  mutex_leave (shuric_mtx);
	  goto cleanup;
	}
  ...
  if (cached_shuric)
    {
      dbg_printf ((" returns %p \"%s\" refs=%d\n",
	      cached_shuric, cached_shuric->_->shuric_type_title, cached_shuric->shuric_ref_count));
    }
  else
    {
...
shuric_t *
shuric_get_typed (caddr_t uri, shuric_vtable_t * vt, caddr_t * err_ret)
{
...
static void
shuric_sentence_tree_to_shot (shuric_t * shu, dk_set_t * shot_list)
{
  ...
  if (NULL != shu->shuric_uri)
    {
      shuric_t **cached_shu_ptr = (shuric_t **) id_hash_get (shuric_global_hashtable, (caddr_t) (&(shu->shuric_uri)));
  ...
  END_DO_SET ()DO_SET (shuric_t *, subshu, &(shu->shuric_imports))
  {
...
static void
shuric_stale_prepare (shuric_t * shu, dk_set_t * obsoletes)
{
  shuric_t **cached_shuric_ptr;
  ...
  END_DO_SET ()if (NULL == shu->shuric_uri)
    return;
...
int
shuric_stale_tree (shuric_t * shu)
{
  dk_set_t obsoletes = NULL;
  mutex_enter (shuric_mtx);
...
int
shuric_is_obsolete_1 (shuric_t * shu, caddr_t * new_ts_ret)
{
...
static int
shuric_subtrees_are_obsolete (shuric_t * shu, dk_set_t * story)
{
...
int
shuric_includes_are_obsolete (shuric_t * shu)
{
  dk_set_t story = NULL;
...
int
shuric_make_import (shuric_t * main, shuric_t * sub)
{
...
int
shuric_make_include (shuric_t * main, shuric_t * sub)
{
...
int
shuric_rollback_import (shuric_t * main, shuric_t * sub)
{
...
int
shuric_rollback_include (shuric_t * main, shuric_t * sub)
{
...
caddr_t
shuric_uri_to_text_default (caddr_t uri, query_instance_t * qi, void *env, caddr_t * err_ret)
{
...
shuric_t *
shuric_scan_relations (shuric_t * haystack_base, caddr_t needle_uri, int scan_mask)
{
...
void
shuric_check_reason_of_forgiveness (shuric_t * shu, int strict)
{
...
void
shuric_validate_refcounters (int strict)
{
...
bif_shuric_list (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  while (hit_next (&it, (caddr_t *) & uri_ptr, (caddr_t *) & shu_ptr))
	...
	continue;
      if (NULL == uri_ptr[0])
	continue;
      if ((NULL != name_mask) && (DVC_MATCH != cmp_like (uri_ptr[0], name_mask, NULL, '\0', LIKE_ARG_CHAR, LIKE_ARG_CHAR)))
	continue;
      type_title = shu_ptr[0]->_->shuric_type_title;
      if ((NULL != type_mask) && (DVC_MATCH != cmp_like (type_title, type_mask, NULL, '\0', LIKE_ARG_CHAR, LIKE_ARG_CHAR)))
	continue;
      dk_set_push (&hits,
	  list (4,
	      box_copy (uri_ptr[0]),
	      box_dv_short_string (type_title), box_num (shu_ptr[0]->shuric_is_stale), box_copy (shu_ptr[0]->shuric_loading_time)));
    }
  mutex_leave (shuric_mtx);
...
void
shuric_on_stale__no_op (struct shuric_s *shuric)
{
#ifdef DEBUG
...
void
shuric_on_stale__cache_remove (struct shuric_s *shuric)
{
#ifdef DEBUG
  shuric_validate_refcounters (0);
#endif
...
caddr_t
shuric_get_cache_key__stub (struct shuric_s *shuric)
{
...
shuric_cache_t *
shuric_cache_alloc__LRU (int size_hint, void *env)
{
...
void
shuric_cache_free__LRU (shuric_cache_t * sc)
{
...
void
shuric_cache_put__LRU (shuric_cache_t * sc, shuric_t * value)
{
  caddr_t key;
  shuric_t **old_ptr;
  shuric_t *old = NULL;
...
void
shuric_cache_empty__LRU (shuric_cache_t * sc)
{
  int ctr;
...
void
shuric_cache_on_idle__LRU (shuric_cache_t * sc)
{
}
