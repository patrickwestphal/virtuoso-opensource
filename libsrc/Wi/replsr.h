...
void trx_repl_log_ddl_index_def (query_instance_t * qi, caddr_t name, caddr_t table, caddr_t * cols, caddr_t * opts);
...
int repl_is_below (repl_level_t log, repl_level_t req);
int session_buffered_chunked_write (dk_session_t * ses, char *buffer, int length);
repl_trail_t *repl_trail_find (repl_acct_t * ra);
void rm_free (repl_message_t * rm);

subscription_t *sub_allocate (char *subscriber, char *account, dk_session_t * sess);
...
/* replsub.c */
void sf_resync_acct (char *account, repl_level_t level, char *subscriber_name, caddr_t name, caddr_t digest);
...
caddr_t sf_resync_replay (char *account, char *subscriber_name, caddr_t name, caddr_t digest);
