...
int
itc_near_random (it_cursor_t * itc, placeholder_t * pl, buffer_desc_t ** buf_ret, ptrlong * n_hits, int is_asc)
{
  ...
  if (PF_OF_DELETED == *buf_ret)
    goto entry_failed;
  if (itc->itc_to_reset <= RWG_WAIT_DATA && (*buf_ret)->bd_page != pl->itc_page)
    {
      page_leave_outside_map (*buf_ret) goto entry_failed;
    }
...
int
itc_il_search (it_cursor_t * itc, buffer_desc_t ** buf_ret, caddr_t * qst, inx_locality_t * il, placeholder_t * pl, int is_asc)
{
  ...
  if (0 && pl && pl->itc_is_registered && n > 3 && n / (hits | 1) < 3)
    {
      res = itc_near_random (itc, pl, buf_ret, &QST_PLONG (qst, il->il_n_hits), is_asc);
  ...
  if (!found && cast_val)
    {
      ...
      if (itc->itc_owned_search_par_fill > 10)
	GPF_T1 ("overflow owned params in inx int");
    }
  ...
  if (itc->itc_bp.bp_value > target)
    {
      ...
      itc_set_search_param (this_itc, this_itc->itc_insert_key->key_n_significant - 1, qst_get (qst, iop->iop_target_ssl),
	  iop->iop_target_dtp);
      return 1;
    }
  return 0;
...
int
inxop_bm_next (inx_op_t * iop, query_instance_t * qi, int op, table_source_t * ts, it_cursor_t * itc)
{
  caddr_t *qst = (caddr_t *) qi;
  int rc;
...
int
inxop_next (inx_op_t * iop, query_instance_t * qi, int op, table_source_t * ts)
{
  ...
  if (IOP_TARGET == op)
    {
      itc->itc_key_spec = iop->iop_ks_full_spec;
      itc_set_search_param (itc, itc->itc_insert_key->key_n_significant - 1, qst_get (itc->itc_out_state, iop->iop_target_ssl),
	  iop->iop_target_dtp);
    }
  else
    itc->itc_key_spec = iop->iop_ks_start_spec;
  ...
  ITC_FAIL (itc)
  {
	...
	rc = itc_il_search (itc, &buf, qst, &iop->iop_il, (placeholder_t *) itc, 0	/*!itc->itc_desc_order */
	    );
	if (DVC_GREATER == rc || DVC_INDEX_END == rc)
	  {
	    itc_page_leave (itc, buf);
...
int
inx_op_and_next (inx_op_t * iop, query_instance_t * qi, int op, table_source_t * ts)
{
  int inx;
...
void
inx_op_source_input (table_source_t * ts, caddr_t * inst, caddr_t * volatile state)
{
  ...
  for (;;)
      ...
      if (ts->ts_main_ks)
	{
	  ...
	  rc = ks_start_search (ts->ts_main_ks, inst, state, main_itc, &main_buf, ts, SM_READ_EXACT);
