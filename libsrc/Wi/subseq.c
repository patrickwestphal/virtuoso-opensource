...
ss_advance (subseq_t * iter, int adv_idx)
{
  if (adv_idx < 0)
    ...
    {
      int curr_arr_idx = ss_search_item (iter->ss_array, iter->ss_state[adv_idx]);
      int advanced = 0;
      if (-1 == curr_arr_idx)
	GPF_T;
      while (curr_arr_idx >= iter->ss_in_state_num + iter->ss_out_state_num - 1)
	{
	  advanced = 1;
	  if (-1 == ss_advance (iter, adv_idx - 1))
	    return -1;
	  curr_arr_idx = ss_search_item (iter->ss_array, iter->ss_state[adv_idx - 1]) + 1;
...
ss_iter_next (subseq_t * iter)
{
  if (!iter->ss_state)
    {
      int inx;
      iter->ss_state = (caddr_t *) dk_alloc_box (iter->ss_in_state_num * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
...
ss_not_in_seq (subseq_t * iter)
{
  ...
  int ns_idx = 0;
  if (!ns)
    ns = iter->ss_not_in_state = (caddr_t *) dk_alloc_box (iter->ss_out_state_num * sizeof (caddr_t), DV_ARRAY_OF_POINTER);
