...
extern xslt_metadata_t *xslt_define (const char *name, int xslt_el_id, xslt_el_fun_t f, int xslt_el_memberofgroups,
    int xslt_el_containsgroups, xsltm_arg_descr_t * arg1, ...);
...
sqlr_new_error_xsltree_xdl (const char *code, const char *virt_code, caddr_t * xmltree, const char *string, ...)
    ...
    caddr_t * err_ret, xml_ns_2dict_t * ns_2dict);
     ...
     extern char *xslt_fmt_print_numbers (char *tail, int tail_max_fill, unsigned *nums, int nums_count, char *format);
