...
static caddr_t
bif_mts_enlist_transaction (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  /* get encoded transaction cookie */
  caddr_t tr_cookie_str = bif_string_arg (qst, args, 0, "mts_enlist_transaction");
  caddr_t tr_cookie;
  unsigned long len;
...
{
  caddr_t param_str = bif_string_arg (qst, args, 0, "mts_status");
...
{
  long param_timeout = bif_long_arg (qst, args, 0, "mts_set_timeout");
...
bif_mts_all_info (caddr_t * qst, caddr_t * err_ret, state_slot_t ** args)
{
  ...
  dbg_printf (("******** cli %x tr %x st %d nthr %d tr2 %x\n",
	  cli, cli->cli_trx, cli->cli_trx->lt_status, cli->cli_trx->lt_threads, qi->qi_trx));
#if 0
  if (!cli->cli_tp_data)
